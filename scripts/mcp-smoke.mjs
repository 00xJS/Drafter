...
async function main() {
  ...
  try {
    ...
    eq(closedRow.data.comments?.[0]?.by, OWNER, 'the closing comment is signed by the member the connection acts for')
    await call('add_comment', { id: task.id, body: 'Table booked for eight' })
    const commented = row(task.id).data.comments?.at(-1)
    eq(commented?.body, 'Table booked for eight', 'add_comment stored the comment')
    eq(commented?.by, OWNER, 'add_comment signs it with the member the connection acts for')
