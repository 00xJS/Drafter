...
export const wearId = (day, rand = randomTen) => `wear~${day}~${rand()}`
...
export function newWear(day, garmentIds, now = new Date().toISOString(), rand = undefined) {
...
export function withPieces(w, garmentIds) {
...
export function liveById(garments) {
...
export const pieceKey = ids => [...new Set(ids)].sort().join('+')
 ...
 */
export function coreKey(ids, byId) {
 ...
 */
export function wearable(ids, byId) {
  return (
    ids.length > 0 &&
    ids.every(id => {
      const g = byId.get(id)
 ...
 */
export function notInUse(ids, byId) {
  const names = [...new Set(ids)].map(id => byId.get(id)).filter(g => !!g?.archivedAt).map(g => g.name)
...
export function unwearable(ids, byId) {
 ...
 */
export function outfitLabel(ids, byId) {
  const known = orderPieces(ids, byId)
  if (known.length === 0) return 'Pieces since deleted'
  const isCore = id => CORE.has(byId.get(id).type)
...
export function notWornLately(garments, ix, days = NOT_WORN_DAYS) {
  const last = g => ix.days.get(g.id)?.[0] ?? ''
  return garments
 ...
 */
export function neverWorn(garments, ix, grace = NEVER_WORN_GRACE_DAYS, dayOf = localDayKey) {
  return garments
...
export function outfitDays(o, ix, byId) {
