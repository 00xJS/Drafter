...
// phone that opens Drafter daily is current enough. The morning's Plan your
// day is the one that repeats by itself, so it comes whether or not the app ran.
...
export function deviceReminders(
  ...
  return [...(opts.local ? buildLocalReminders(data.tasks, data.people, data.places, data.meals, now, 30, opts) : []), ...(plan ? [plan] : [])]
}
