...
describe('purge tombstones survive every sanitizer', () => {
  ...
  const kinds = ['task', 'project', 'person', 'place', 'recipe', 'template', 'meal', 'grocery', 'journal', 'review', 'calendar', 'note', 'garment', 'outfit', 'wear'] as const
  it('a content-free tombstone of any kind round-trips with deletedAt intact', async () => {
    ...
    for (const kind of kinds) {
      const id = kind === 'journal' ? 'journal~2026-09-07~abcd' : kind === 'wear' ? 'wear~2026-09-07~abcd' : `${kind}-1`
    ...
    expect((sanitizeItem(purgeTombstone('wear', 'wear~2026-09-07~abcd', now)) as { date: string }).date).toBe('2026-09-07')
  })
  ...
  it('still refuses a live record that lacks its identity field', () => {
    ...
    expect(sanitizeItem({ kind: 'outfit', id: 'o', updatedAt: now, garmentIds: [] })).toBeNull()
    expect(sanitizeItem({ kind: 'wear', id: 'w', updatedAt: now, garmentIds: ['g'] })).toBeNull()
  })
})
