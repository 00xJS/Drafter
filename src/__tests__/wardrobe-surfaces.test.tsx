...
describe('one check for a planned look', () => {
  ...
  it('is asked in the one place every figure is read from', () => {
    const src = read('../wardrobe.ts')
    const index = src.slice(src.indexOf('export function wearIndex'), src.indexOf('// ---- per piece'))
    expect(index).toContain('isPlanned(w)')
    // called once, by wearIndex: switching planned looks on is that one helper
    expect(src.match(/\bisPlanned\(/g)).toHaveLength(1)
  })
})
...
describe('cost per wear', () => {
  it('divides each price by the days worn, best value first and the never worn last, over the pieces with a price', () => {
    ...
    const cost = costPerWear(g, wearIndex(ws, TODAY))
  ...
  it('has nothing to say with no prices, or no wears', () => {
    expect(costPerWear([piece('tee', 'top')], wearIndex([look(ago(1), ['tee'])], TODAY)).rows).toEqual([])
    expect(costPerWear([piece('tee', 'top', { price: 10 })], wearIndex([], TODAY)).perWear).toBeUndefined()
  })
})
