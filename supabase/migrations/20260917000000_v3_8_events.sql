-- v3.8: calendar entries you write yourself (kind = 'event').
--
-- Until now every event on the calendar was a read-only projection of an
-- external feed: `calendar` rows are SUBSCRIPTIONS, and /api/calendars returns
-- occurrences that are never stored. So there was no way to block out 2-3pm.
-- An `event` row is the missing piece: a title with a real start and end.
--
-- Ship this BEFORE any client writes an event. An unknown kind is rejected by
-- sync_posts, and the client keeps retrying it, so the row would sit on one
-- device looking saved and reach nothing else — which is exactly the shape of
-- the September outage.
--
-- Events are HOUSEHOLD-visible, like tasks, meals and places, not owner-only
-- like journal/review/calendar. A subscription is private because it carries a
-- feed token; a block of time on a shared family calendar is the opposite —
-- the whole point is that the other person can see the evening is taken.
    ...
    for item in select value from jsonb_array_elements(incoming) loop
      ...
      begin
        ...
        if (item ? 'id') and (item ? 'updatedAt')
           and kind in ('task', 'project', 'calendar', 'person', 'place', 'review', 'template', 'recipe', 'meal', 'grocery', 'journal', 'event')
           and (not item ? 'status'
