-- v3.4: places (kind = 'place') join the posts table; tasks may carry placeId.
-- Copy the newest sync_posts body (synced_at) and append 'place' to the allow-list.
-- Ship this before any client writes place rows — unknown kinds are dropped silently.
    ...
    for item in select value from jsonb_array_elements(incoming) loop
      ...
      begin
        ...
        if (item ? 'id') and (item ? 'updatedAt')
           and kind in ('task', 'project', 'calendar', 'person', 'place', 'review', 'template')
           and (not item ? 'status'
