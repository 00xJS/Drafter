-- v3.2: people (kind = 'person') join tasks, projects and calendars in the
-- same table; tasks gain peopleIds. Only the kind allow-list changes.
    ...
    for item in select value from jsonb_array_elements(incoming) loop
      begin
        ...
        if (item ? 'id') and (item ? 'updatedAt')
           and kind in ('task', 'project', 'calendar', 'person')
           and (not item ? 'status'
