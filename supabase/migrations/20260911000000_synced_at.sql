-- Sync cursor on server arrival time (synced_at), not the client's updatedAt.
-- Offline / clock-behind edits pushed late still reach peers whose cursor
-- already passed the record stamp. Rejected writes are listed so the client
-- can drop them from the dirty set instead of clamping the cursor forever.

alter table public.posts
  add column if not exists synced_at timestamptz not null default clock_timestamp();

update public.posts set synced_at = updated_at where synced_at is distinct from updated_at;

create index if not exists posts_synced_at_idx on public.posts (synced_at);
...
begin
  if jsonb_typeof(incoming) = 'array' then
    for item in select value from jsonb_array_elements(incoming) loop
      ...
      if id is not null and not accepted then
