-- v3.1: calendar subscriptions live in the same table as a third record kind.
-- { kind: "calendar", id, name, url, color, enabled, createdAt, updatedAt }
    ...
    for item in select value from jsonb_array_elements(incoming) loop
      begin
        ...
        if (item ? 'id') and (item ? 'updatedAt')
           and kind in ('task', 'project', 'calendar')
           and (not item ? 'status'
