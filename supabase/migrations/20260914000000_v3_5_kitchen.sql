-- v3.5: kitchen kinds (recipe, meal, grocery) join the posts table.
-- Copy the newest sync_posts body (places) and append the three kinds.
-- Ship this before any client writes kitchen rows — unknown kinds are dropped silently.
    ...
    for item in select value from jsonb_array_elements(incoming) loop
      ...
      begin
        ...
        if (item ? 'id') and (item ? 'updatedAt')
           and kind in ('task', 'project', 'calendar', 'person', 'place', 'review', 'template', 'recipe', 'meal', 'grocery')
           and (not item ? 'status'
