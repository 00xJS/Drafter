...
- **Local first.** Records live on the device and sync to Supabase, so the app works offline. Changes from the other device arrive within seconds while the app is open. Edits from two devices merge field by field; when both changed the same field, the edit saved first is kept, and the other device's toast offers **Keep mine**.
...
- **Backups.** Each night the server saves an encrypted snapshot of every account, journal included, in private storage and keeps the newest 14; only the site owner can download them, from Admin → Backups. Anyone can export a JSON file from Settings → Data.
  - **Off-site copy.** The owner's Mac copies the encrypted snapshots into iCloud Drive every night at 03:30 (`Drafter Backups/<account>/`) and keeps 60 days of them; a snapshot written before encryption was on never goes there. `scripts/install-offsite-backup.sh` sets that up as a LaunchAgent (`--uninstall` removes it), running a copy of the script it pins under `~/Library/Application Support/Drafter/offsite` rather than the checkout (install again to refresh it). Each run adds a line to `~/Library/Logs/drafter-backup-offsite.log`; a failed night shows a macOS notification, and so, once a week, does a newest copy more than 3 days old.
...
npm run e2e:cloud    # browser tests against a local Supabase stack: sign-in, sync and Realtime (below)
```

The cloud lane builds the app in cloud mode against a local stack (`supabase start`, which needs Docker) and drives two members of one household in two browsers: signing in and out, a task and its completion reaching the other within seconds, a journal entry and a private task staying with their writer, edits made offline, and both devices renaming one task. It reads `E2E_SUPABASE_URL`, `E2E_SUPABASE_ANON_KEY`, `E2E_SUPABASE_SERVICE_ROLE_KEY` and `E2E_DATABASE_URL`, which are `supabase status -o env`'s `API_URL`, `ANON_KEY`, `SERVICE_ROLE_KEY` and `DB_URL`, and needs `psql`. Without them it skips; it refuses any address but `localhost` or `127.0.0.1`, so it can't be pointed at production.
...
- **CI.** Every push and pull request runs lint and `check`, both smoke tests, the browser tests and the Deno bot's checks on GitHub, and the cloud lane against a stack started on the runner (advisory until it has proven itself: it holds up no deploy). The repository is public, so each action is pinned to a commit, and Dependabot opens weekly update pull requests (npm within each major in one, Capacitor on its own, anything under a week old left for later).
