...
A local-first personal project manager. Projects hold tasks with due dates, priorities, checklists and a comment trail; a roadmap lays projects and milestones out over time; the Today page tells you what's overdue, due today and due this week. People and places track who you saw and where you went. A Kitchen tab holds recipes, the week's meals (cooked or eaten out) and a grocery list, and those meals show on the calendar too. A daily journal sits beside the weekly review and feeds it. Tasks can link to GitHub issues, pull requests, repos or Projects boards and show their live state.
...
**Work days.** Mark where you are working and your hours: a **work day** is set to 🏠 Home or 🏢 Office with a start and an end, from the calendar's **+** (*Work day*), a day's *Set work day*, or the event editor's *Work day* switch. Pick weekdays under *Repeat on* (Monday and Friday at home, say) and a number of weeks, and each of those days is created separately, so any one of them can be changed on its own — the Friday you go in instead. A work day is a property of the day rather than something happening on it, so it shows as a small green 🏠 or blue 🏢 badge with the hours on the month cell, the week header and the day sheet, and it is left out of the day's item count. It is never busy time: Google gets it as free, Outlook as **working elsewhere** (home) or free (office), and the ICS feed marks it transparent, so it never blocks anyone booking you during your working day.

**Bills.** Track what the household pays: bills, credit cards, subscriptions, loans. A bill is a task with an amount, so tick *This is a bill or payment* in any task (or press **+ Bill** on the Bills tab, which starts one repeating monthly), choose its kind, who is paid and whether it is paid automatically, and set the **Amount due**. It then sits on its due date in the calendar as 🧾, 💳, 🔁 or 🏦 with its amount, reminds you like any task, shows on Today, and repeats monthly, quarterly or yearly. A bill falls due on its own day however early or late you paid it, and one due on the 31st goes 31 Jan → 28 Feb → 31 Mar rather than settling on the 28th. Marking it done with nothing under **Paid** records the amount due as paid. The **Bills** tab (a row in More on a phone) shows a month at a time: what is overdue, what is still to come, what has been paid, and what an average month costs across every repeating payment. Amounts appear only inside Drafter — the Google and Outlook mirrors and the ICS feed send the title you typed and nothing more. Figures are shown in pounds.
