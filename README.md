...
- **Tasks → Notes** — your notes and every project's notepad in one searchable list, pinned notes first; open one to write in it (rich text with inline photos), and *All notes* returns to the list. The ☐ Task button turns the selected line into a task.
...
- **Calendar → Timeline** — from the day your account began to today, fitted to the width of the window: projects as bars across months (dashed when the span is inferred — set start/target dates to pin it), milestones as ◆, due tasks as dots, a today line. Click anything to open it.
