...
- **Home → Today** — a **briefing strip** first: a greeting for the hour, then a tile for each fact the day has — the weather, today's work mode and hours, events (with the first start time), habits done of due, tonight's dinner. Weather is opt-in and comes from Open-Meteo with no key: nothing is fetched and no permission prompt appears until you press *Enable weather*, your location goes to Open-Meteo and nowhere else, nothing is stored server-side, and turning it off forgets the location. Below that: overdue, due today, this week, the Inbox, Next up, this week's Top 3, tonight's dinner, people due a catch-up, upcoming birthdays and anniversaries, and (on Sunday) last week's review excerpt; one-tap complete and defer.
...
- **Tasks → Notes** — the selected project's notepad (rich text with inline photos), or an index of all projects' notes. The ☐ Task button turns the selected line into a task in that project.
...
- **Native patterns.** The same five tabs, but iOS large titles, editors as
  ...
  a frosted tab bar. Gated on `html.native`, so the web build is untouched.
